...
function generateErrorMailContent(jsonData) {
    ...
    // Об'єднати всі поля в один заголовок для таблиці (MD Header Row)
    const allFieldsHeader = '| ' + [...rootFields, ...debtorFields].join(' | ') + ' |';
    const allFieldsHeaderKeys = '| ' + [...rootKeys, ...debtorKeys].join(' | ') + ' |';

    // Рядок розділювача Markdown ( | --- | --- | ... | )
    const mdSeparator = '|' + [...rootFields, ...debtorFields].map(() => ' --- ').join('|') + '|';
    ...
    const dataRows = (jsonData.errors || []).map(error => {
        ...
        // Об'єднання всіх значень в один рядок у форматі Markdown таблиці (| value1 | value2 | ... |)
    ...
    const bodyContent = [
        ...
        ``,
        // Заголовок Markdown таблиці
        allFieldsHeader,
        allFieldsHeaderKeys,
        // Роздільник Markdown таблиці
        mdSeparator,
        // Рядки даних Markdown таблиці
        ...dataRows
    ].join('\n');
