 ...
 * @WHERE src/nodered/webs/projects/prepareFlowSQL-9.js
 * @AUTHOR stardust
 * @FLOW SQLs
 * @VERSION 0.0.9
 ...
 * 2025-09-28 10:16:34 improve a select clause
*/
//--------------------------------------------------------------
var hasError = false;
...
function prepare_select(payload, nonl) {
    ...
    if (payload.join) {
        for (var item of payload.join) {
            sql += `JOIN ${item.table} ${item.alias} ON ${item.alias}.${item.field} = ${payload.alias}.${item.reffer}${nl}`;
        }
    }
    sql += `${pref}WHERE 1=1`;
    if (payload.where) {
