...
-- DROP FUNCTION sc_GetVersionHistory;
CREATE OR REPLACE FUNCTION sc_GetVersionHistory(
	IN pTaskCode varchar(255)
)
RETURNS TABLE (
        TaskCode VARCHAR
        , Versionq VARCHAR
...
BEGIN 
    -- Treść procedury (instrukcje SQL)
 RAISE NOTICE 'GO+ %', pTaskCode;

 RETURN QUERY
	SELECT dob.taskcode, dbh.Version , dbh.Build, dbh.Created
	FROM dbObjects dob
	JOIN dbHistory dbh ON dbh.objectid = dob.id
	WHERE dob.TaskCode = pTaskCode;
	--GET DIAGNOSTICS Build := ROW_COUNT;
END; 
$procedure$;

SELECT * FROM sc_GetVersionHistory('sc_GetVersion');
